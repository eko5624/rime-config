...
local function QueryLunarInfo(date)
	...
	if string.match(str,"^(20)%d%d+$")~=nil or string.match(str,"^(19)%d%d+$")~=nil then
        ...
        if LunarGz ~= nil and LunarDate ~= nil then
            ...
            -- 提取农历日期字符串（确保非 nil）
            local lunarStr = LunarDate.lunarDate_4 or "未知农历日期"
            result = {
